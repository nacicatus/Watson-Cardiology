...
class MessagesViewController: JSQMessagesViewController {
    ...
    private var discussFurtherButton: UIButton!
    private var seeEvidenceButton: UIButton!
    ...
    private var incomingBubble: JSQMessagesBubbleImage = JSQMessagesBubbleImageFactory().incomingMessagesBubbleImageWithColor(UIColor.cyanColor())
    private var outgoingBubble: JSQMessagesBubbleImage = JSQMessagesBubbleImageFactory().outgoingMessagesBubbleImageWithColor(UIColor.lightGrayColor())
    ...
    private let kSegueIdentifier: String = "toEvidence"
    ...
    private func customizeContentView() {
        ...
        self.inputToolbar.contentView.textView.font = UIFont.italicSystemFontOfSize(16.0)
    ...
    private func instantiateSTT() {
        ...
        guard let credentialsURL = bundle.pathForResource("WatsonCardiology", ofType: "plist") else {
            return
        }
    ...
    private func setupSendButton() {
        ...
        sendButton.titleLabel?.font = UIFont.boldSystemFontOfSize(15.0)
    ...
    private func subscribeToDecisionButtonNotifications() {
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(MessagesViewController.discussFurtherButtonPressed(_:)), name: "discussFurtherButtonPressed", object: nil)
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(MessagesViewController.seeEvidenceButtonPressed(_:)), name: "seeEvidenceButtonPressed", object: nil)
    }
    
    @objc private func discussFurtherButtonPressed ( object: AnyObject) {
        hideDecisionsView()
        reachedEndOfConversation = false
    }
    
    @objc private func seeEvidenceButtonPressed (object: AnyObject) {
...
//        if let managementVC = segue.destinationViewController as? ManagementDetailViewController {
//            managementVC.displayText = self.viewModel.watsonEntities
//
//        }
//    }
}
